...
@section('content')
    ...
    {{-- ===== TAB NAVIGATION ===== --}}
    <div style="display:flex; gap:4px; margin-bottom:16px; border-bottom:1px solid var(--card-border);">
        <button type="button" class="rf-tab-btn rf-tab-active" data-tab="detail">
            Detail Laporan
        </button>
        <button type="button" class="rf-tab-btn" data-tab="tindak-lanjut">
            Riwayat Tindak Lanjut
            @if($laporan->tindakLanjuts->count() > 0)
                <span style="background:#f59e0b; color:#1e1b16; font-size:11px; font-weight:700; padding:1px 7px; border-radius:99px; margin-left:6px;">
                    {{ $laporan->tindakLanjuts->count() }}
                </span>
            @endif
        </button>
    </div>
    ...
    {{-- ===== TAB: DETAIL LAPORAN ===== --}}
    <div id="rf-tab-detail" class="rf-tab-content">
        <div style="display:grid; grid-template-columns: 2fr 1fr; gap:16px; align-items:start;">

            {{-- KOLOM KIRI: Data + Deskripsi + Peta --}}
    ...
    {{-- ===== TAB: RIWAYAT TINDAK LANJUT ===== --}}
    <div id="rf-tab-tindak-lanjut" class="rf-tab-content" style="display:none;">
        <div class="card">
            @if (session('success'))
    ...
    @push('scripts')
    <script>
        document.querySelectorAll('.rf-tab-btn').forEach(function (btn) {
            btn.addEventListener('click', function () {
                document.querySelectorAll('.rf-tab-btn').forEach(b => b.classList.remove('rf-tab-active'));
                document.querySelectorAll('.rf-tab-content').forEach(c => c.style.display = 'none');

                btn.classList.add('rf-tab-active');
                document.getElementById('rf-tab-' + btn.dataset.tab).style.display = 'block';
            });
        });
    </script>
    @endpush
